...
const Button = ({manageData}) => {
  ...
  useEffect(() => {
    ...
    axios.get(API).then(res => setData([res.data])).catch(error => alert(error));
  ...
  return (
    ...
    <div className='btn' onClick={() => { if (ishidden) handleCollapse(); }} style={{borderRadius:5}}>
        <img src={adjust} alt='' className='adjust-icon' />
        <h4 style={{fontFamily:'Arial'}}>Display</h4>
        <img src={carret} alt='' />
        <div className='options hidden'>
          <div className='grouping'>
            <p style={{fontFamily:'Arial'}}>Grouping</p>
            <select onChange={handleGrouping} style={{fontWeight:'bold'}}>
              {/* <option>Select</option> */}
              <option style={{fontFamily:'Arial',fontWeight:'bold'}}>Status</option>
              <option style={{fontFamily:'Arial',fontWeight:'bold'}} >User</option>
              <option style={{fontFamily:'Arial',fontWeight:'bold'}}>Priority</option>
            </select>
          </div>
          <div className='ordering'>
            <p style={{fontFamily:'Arial'}}>Ordering</p>
            <select onChange={handleOrdering} style={{fontWeight:'bold'}}>
              {/* <option>Select</option> */}
              <option style={{fontFamily:'Arial',fontWeight:'bold'}}>Priority</option>
              <option style={{fontFamily:'Arial',fontWeight:'bold'}}>Title</option>
            </select>
          </div>
        </div>
      </div>
